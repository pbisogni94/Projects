#!/usr/bin/env python3
# Student name: Patrick Bisogni
# Student number:
# teach.cs login username: bisognip
...
class PartialParse(object):
    ...
    def complete(self):
        ...
        """
        # ****BEGIN YOUR CODE****
        # when we've read through the entire sentence (exhausted the buffer)
        # and the stack length is 1, the partialparse is complete
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        if transition_id == self.shift_id:
            # put the firstmost word on the buffer on the stack
            self.stack.append(self.next)
            # move the tracker to the next word in the buffer
            self.next += 1
        elif transition_id == self.left_arc_id:
            head_pos = -1
            dep_pos = -2
            idx_head = self.stack[head_pos]
            idx_dep = self.stack[dep_pos]
            arc = (idx_head, idx_dep, deprel)
            # remove the second most recent item in the stack
            self.stack = self.stack[:dep_pos] + self.stack[head_pos:]
            self.arcs.append(arc)
        elif transition_id == self.right_arc_id:
            head_pos = -2
            idx_head = self.stack[head_pos]
            # the firstmost item on the stack is the dependant in a right arc
            # and we also want to remove it from the stack
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        # ****BEGIN YOUR CODE****
        all_deps = [
            arc[1] for arc in self.arcs
            if arc[0] == sentence_idx
        ]
        # order the deps (indices) from smallest to largest; left to right
        all_deps.sort()
        # get up to n dependencies
        deps = all_deps[:n]
        # ****END YOUR CODE****
        return deps
    ...
    def get_n_rightmost_deps(self, sentence_idx, n=None):
        ...
        # order the deps (indices) from largest to smallest; right to left
        all_deps.sort(reverse=True)
        # get up to n dependencies
        deps = all_deps[:n]
        # ****END YOUR CODE****
        return deps
    ...
    def get_oracle(self, graph: DependencyGraph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        # ****BEGIN YOUR CODE****
        if len(self.stack) == 1:
            # this is the first transition, since we error if PP is complete
            transition = self.shift_id
            deprel_label = None
        elif self.stack[-2] in get_deps(self.stack[-1], graph):
            # if the second most recently added item depends on the first,
            # there is a left arc between them. We left arc as soon as we can.
            transition = self.left_arc_id
            # dependent's relation label
            deprel_label = get_deprel(self.stack[-2], graph)
        elif self.stack[-1] in get_deps(self.stack[-2], graph):
            # we have that the most recently added item on the stack is
            # dependent on the second most recently added item, BUT we
            # can only draw this right-arc IFF all dependents of the most
            # recently added item have already been processed (ie. exist
            # in self.arcs as idx_dep), otherwise we will pop it off,
            # preventing its dependents from properly having their head
            # labelled and being removed
            first_item_dependents = get_deps(self.stack[-1], graph)
            all_deps_accounted_for = True
            for dpd in first_item_dependents:
                existing_arcs = [
                    arc for arc in self.arcs
                    # arc[1] bc we want idx_dep in (idx_head, idx_dep, deprel)
                    if arc[1] == dpd
                ]
                if len(existing_arcs) == 0:
                    all_deps_accounted_for = False
                    break
            if all_deps_accounted_for:
                # All it's dependents' heads have been accounted for, we can
                # draw a right arc and remove it
                transition = self.right_arc_id
                deprel_label = get_deprel(self.stack[-1], graph)
            else:
                # We must continue shifting until we've established the heads
                # of all it's dependents
                transition = self.shift_id
                deprel_label = None
        else:
            # There is no relation between self.stack[-1] and self.stack[-2],
            # so we shift
            transition = self.shift_id
            deprel_label = None
        # ****END YOUR CODE****
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # ****BEGIN YOUR CODE****
    partial_parses = [
        PartialParse(sentence) for sentence in sentences
    ]
    unfinished_parses = partial_parses.copy()
    while len(unfinished_parses) != 0:
        td_pairs = model.predict(unfinished_parses[:batch_size])
        completed_parses = []
        # use len(td_pairs) here so we know how many elements are in the slice
        # [:batch_size]
        for i in range(len(td_pairs)):
            try:
                unfinished_parses[i].parse_step(td_pairs[i][0], td_pairs[i][1])
            except ValueError:
                # mark this parse to be removed as it's "stuck"
                completed_parses.append(unfinished_parses[i])
            else:
                if unfinished_parses[i].complete:
                    completed_parses.append(unfinished_parses[i])
        for completed_parse in completed_parses:
            unfinished_parses.remove(completed_parse)

    arcs = [
        partial_parse.arcs for partial_parse in partial_parses
    ]
    # ****END YOUR CODE****
    return arcs
