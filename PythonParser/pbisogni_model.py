...
class ParserModel(nn.Module):
    ...
    def create_net_layers(self) -> None:
        ...
        """
        # ****BEGIN YOUR CODE****
        input_sample_size = (self.config.n_word_features + self.config.n_tag_features +
                             self.config.n_deprel_features) * self.config.embed_size
        self.hidden_layer = nn.Linear(input_sample_size, self.config.hidden_size)
    ...
    def reshape_embedded(self, embedded_batch: torch.Tensor) -> torch.Tensor:
        ...
        """
        # ****BEGIN YOUR CODE****
        sizes = embedded_batch.size()
        reshaped_batch = embedded_batch.reshape(sizes[0], sizes[1] * sizes[2])
        # ****END YOUR CODE****
    ...
    def get_concat_embeddings(self, word_id_batch: torch.Tensor,
        ...
        """
        # ****BEGIN YOUR CODE****
        word_embeddings = self.word_embed(word_id_batch)
        tag_embeddings = self.tag_embed(tag_id_batch)
        deprel_embeddings = self.deprel_embed(deprel_id_batch)
        # Need to be concatenated in dimension 1 since dimension 0 is =
        # consistent (batch_size)
        x = torch.cat(
            (self.reshape_embedded(word_embeddings),
             self.reshape_embedded(tag_embeddings),
             self.reshape_embedded(deprel_embeddings)),
            dim=1,
        )
        # ****END YOUR CODE****
        return x
    ...
    def forward(self,
        ...
        pred = self.output_layer(
            F.dropout(F.relu(self.hidden_layer(x)), self.config.dropout, self.training)
        )
        # ****END YOUR CODE****
        return pred
